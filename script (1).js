...
// Contador de visitas: usa um serviço gratuito (sem cadastro) pra contar
// visitas de todo mundo. Se ele estiver fora do ar, cai num contador local
// (só deste navegador) pra não deixar o número travado em "...".
(function contarVisita(){
  const el = document.getElementById('visitor-count');
  if(!el) return;

  // Troque esse texto por algo único (ex: seu nome + site) se for reaproveitar
  // esse código em outro projeto, pra não misturar a contagem com a de outra pessoa.
  const CHAVE_CONTADOR = 'willy-cesar-moreira-site-pessoal';

  fetch('https://countapi.mileshilliard.com/api/v1/hit/' + encodeURIComponent(CHAVE_CONTADOR))
    .then(r => r.json())
    .then(data => { el.textContent = data.value; })
    .catch(() => {
      const total = parseInt(localStorage.getItem('visitas-local') || '0', 10) + 1;
      localStorage.setItem('visitas-local', total);
      el.textContent = total + ' (neste navegador)';
    });
})();

...
/* ---------- formulário "Conte sua história" ---------- */
function initStoryForm() {
  const form = document.getElementById('storyForm');
  if (!form) return;

  const destinatario = document.querySelector('.contact-list a[href^="mailto:"]')?.getAttribute('href')?.replace('mailto:', '') || 'willymoreira23@gmail.com';

  form.addEventListener('submit', (e) => {
    e.preventDefault();

    const nome = document.getElementById('storyName').value.trim();
    const contato = document.getElementById('storyContact').value.trim();
    const historia = document.getElementById('storyText').value.trim();
    ...
    const assunto = `Nova história para música — ${nome || 'Anônimo'}`;
    const corpo =
    ...
    const msg = document.getElementById('storySentMsg');
    if (msg) {
